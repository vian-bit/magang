...
<div class="bg-white rounded-lg shadow p-4 md:p-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-4 md:mb-6">
        <div class="bg-blue-100 p-4 md:p-6 rounded-lg">
            <div class="text-2xl md:text-3xl font-bold text-blue-600">{{ $totalUsers }}</div>
            <div class="text-xs md:text-base text-gray-600">Total Users</div>
        </div>
        <div class="bg-green-100 p-4 md:p-6 rounded-lg">
            <div class="text-2xl md:text-3xl font-bold text-green-600">{{ $todaySchedules }}</div>
            <div class="text-xs md:text-base text-gray-600">Today's Schedule</div>
        </div>
        <div class="bg-yellow-100 p-4 md:p-6 rounded-lg">
            <div class="text-2xl md:text-3xl font-bold text-yellow-600">{{ $todayPresent }}</div>
            <div class="text-xs md:text-base text-gray-600">Present</div>
        </div>
        <div class="bg-red-100 p-4 md:p-6 rounded-lg">
            <div class="text-2xl md:text-3xl font-bold text-red-600">{{ $todayLate }}</div>
    ...
    <div class="mt-4 md:mt-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
            <a href="{{ route('users.create') }}" class="bg-blue-500 text-white p-4 md:p-6 rounded-lg text-center hover:bg-blue-600 transition-colors">
                <div class="text-2xl md:text-3xl mb-2">👥</div>
                <div class="text-sm md:text-base">Add User</div>
            </a>
            <a href="{{ route('shifts.index') }}" class="bg-green-500 text-white p-4 md:p-6 rounded-lg text-center hover:bg-green-600 transition-colors">
                <div class="text-2xl md:text-3xl mb-2">⏰</div>
                <div class="text-sm md:text-base">Manage Shifts</div>
            </a>
            <a href="{{ route('schedules.calendar') }}" class="bg-yellow-500 text-white p-4 md:p-6 rounded-lg text-center hover:bg-yellow-600 transition-colors">
                <div class="text-2xl md:text-3xl mb-2">📅</div>
                <div class="text-sm md:text-base">Manage Schedule</div>
            </a>
            <a href="{{ route('attendances.export') }}" class="bg-purple-500 text-white p-4 md:p-6 rounded-lg text-center hover:bg-purple-600 transition-colors">
                <div class="text-2xl md:text-3xl mb-2">📊</div>
                <div class="text-sm md:text-base">Export Report</div>
            </a>
        </div>
    </div>
</div>
...
checkWAStatus();
setInterval(checkWAStatus, 10000); // refresh tiap 10 detik
</script>
@endsection
